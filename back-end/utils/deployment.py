...
from the product's domain. Five pieces of configuration are what keep this app's public surface from
being an open relay - or, in the last case, from being a locked door - and
``verify_public_endpoint_defenses`` (``app.py``) requires all five:
...
- ``RESEND_API_KEY``, which delivers the one-time code that is an applicant's only way in.

All five fail *silently* when unset - the captcha passes every caller; the limits key on the
proxy's address, which is one shared budget for the whole world; a token signed with a published
key is a token anyone can write; a credentialed CORS policy with no origin list reflects whatever
``Origin`` the caller sent; and the login code is not sent, which nothing on that path may report,
because a response that distinguished a send from a non-send would disclose who has applied.
