...
test.describe('Public applicant flow', () => {
  ...
  test('a first-time applicant keeps their answers through the sign-in redirect, and the promised save happens', async ({
    page,
  }) => {
    ...
    // Back on the page they came from, with their answers on it, and the save the button's own
    // label promised carried out without asking them to press it a second time.
  ...
  test('answers stranded by an expired session are not handed to the next applicant who signs in', async ({
    ...
    // purpose - that is the one moment they are most needed - so they are still in this tab when
    // the applicant walks away from it.
    ...
    // The first applicant's business details are not theirs to read...
    await expect(apply.input('business_name')).toHaveValue('');
    await expect(apply.input('product_type')).toHaveValue('');
    // ...and this page finishes the save its button promised, so a draft it accepted would have
    // been written onto this applicant's application with nothing pressed at all.
