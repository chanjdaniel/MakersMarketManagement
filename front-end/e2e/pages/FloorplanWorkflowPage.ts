...
export class FloorplanWorkflowPage {
  ...
  constructor(page: Page) {
    ...
    this.tableTypeWidthInput = page.getByTestId('floorplan-table-type-width-input').locator('input.p-inputtext').first();
    this.tableTypeHeightInput = page.getByTestId('floorplan-table-type-height-input').locator('input.p-inputtext').first();
  ...
  async autoPlaceTables(): Promise<void> {
    ...
    await expect(this.autoPlaceError).not.toBeVisible({ timeout: 3000 }).catch(() => {});
  }
   ...
   */
  async snapshotPlacedTables(): Promise<any[]> {
   ...
   */
  async restorePlacedTables(tables: any[]): Promise<void> {
  ...
  async groupAllTablesIntoSection(
    sectionName: string,
    locationName: string,
  ): Promise<void> {
    await this.page.evaluate(
      ({ name, location }) => {
        ...
        const tableIds = store.placedTables.map((t: any) => t.id);
