...
const hasActiveFilters = computed(
    () => Boolean(dateFilter.value) || Boolean(sectionFilter.value) || Boolean(tierFilter.value) || Boolean(choiceFilter.value),
);
...
const groupedRows = computed((): DateGroup[] => {
    ...
    for (const [date, sectionMap] of dateMap.entries()) {
        ...
        for (const section of sections) {
            section.rows.sort((a, b) => a.tableCode.localeCompare(b.tableCode, undefined, { numeric: true }));
        }
                ...
                <template v-else-if="allRows.length > 0">
                    <div class="filter-bar">
                        ...
                        <div class="counts-row">
                            ...
                            <span class="count-badge count-badge--assigned">{{ statusCounts.assigned }} assigned</span>
                            <span class="count-badge count-badge--partial">{{ statusCounts.partial }} partial</span>
                        ...
                        >
                            ...
                            >
                                <h3 class="section-heading">
                                    ...
                                    <span v-if="sectionGroup.location" class="section-heading-meta">{{ sectionGroup.location }}</span>
                                    <span v-if="sectionGroup.tier" class="section-heading-meta">{{ sectionGroup.tier }}</span>
                                </h3>
                                ...
                                <ul class="table-list">
                                    ...
                                    >
                                        <div class="table-row-head">
                                            ...
                                            <span
                                                class="choice-badge"
                                                :class="row.tableChoice.toLowerCase().includes('full') ? 'choice-badge--full' : 'choice-badge--half'"
                                            >
                                                {{ row.tableChoice }}
                                            </span>
                                        ...
                                        <div class="table-row-assignment">
                                            ...
                                            <template v-else-if="rowStatus(row).isFull">
                                                <span class="assignment-email assignment-email--full">{{ rowStatus(row).leftEmail }}</span>
                                            </template>
                                            <template v-else>
                                                <div class="half-slot">
