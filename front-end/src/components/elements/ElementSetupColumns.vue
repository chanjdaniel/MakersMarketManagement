...
<template>
  <div class="container" ref="container">
    <div class="column-titles row-container" ref="columnTitles">
      ...
      <div
        class="info-icon-wrapper title-row-item"
        @mouseenter="infoVisible = true"
        @mouseleave="infoVisible = false"
      >
        <IconInfo class="info-icon" />
        <div v-if="infoVisible" class="info-popup">
          <p class="info-text">
            Click the checkbox if you would like to include this column in the vendor info view.
            <br /><br />

            It's recommended to include important info like names and contact info, and exlcude
            columns for miscellaneous information like additional comments etc.<br /><br />

            It's possible to come back and change these settings later. All data will be available
            for the assignment process, this is just for hiding unimportant information from view.
          </p>
        </div>
      </div>
    </div>
        ...
        <div class="row-item text-item">
          <h4 class="setup-row-colname">
            <textarea
              ...
              :ref="
                (el) => {
                  textareas[index] = el as HTMLTextAreaElement | null;
                }
              "
        ...
        <div class="row-item enum-item">
          <select class="datatype-dropdown">
            <optgroup class="datatype-dropdown">
              ...
              <option
                class="display-list"
                v-for="value in colValues[index]"
                :key="value"
                :value="value"
              >
                {{ value }}
